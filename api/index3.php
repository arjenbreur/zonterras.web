  ...
  <head>
    ...
    function OnLocalSearch() {
      ...
      for (var i = 0; i < gLocalSearch.results.length; i++) {
		...
		// save coord in array
		coords[coords.length]={lat:gLocalSearch.results[i].lat,lng:gLocalSearch.results[i].lng};
	  ...
	  // loop results, requesting sunshine for each result
	  for(var i=0; i<gCurrentResults.length;i++){
		//alert('ajax for '+gCurrentResults.lat);
		// send ajax request
		$.ajax({
		  url: "ajax.php",
		  data: coords,
		  success: function(data,successTxt){
			if(successTxt=='success' && data=='true'){
				// location is in the sun right now
				// update icon to sun icon
	...
	function OnShadeRequest(oResult){
